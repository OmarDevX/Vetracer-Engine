...
pub fn process_physics(&mut self, delta_time: f32, spheres: &mut [Sphere]) {
    ...
    fn update_orientation(&mut self, delta_time: f32) {
        ...
        if angle != 0.0 {
            ...
            self.orientation = Sphere::quaternion_multiply(self.orientation, delta_orientation);
        }
    }
...
fn resolve_collision(&mut self, other: &mut Sphere) {
    if self != other {
        let distance = Sphere::distance_between_spheres(self, other);
        ...
        if penetration_depth > 0.1 {
            ...
            if !self.is_static && other.is_static {
                ...
                self.position[0] -= collision_normal[0] * penetration_depth;
                self.position[1] -= collision_normal[1] * penetration_depth;
                self.position[2] -= collision_normal[2] * penetration_depth;
            } else if !other.is_static && self.is_static {
                // Static sphere hits non-static sphere, adjust non-static sphere's position
                other.position[0] += collision_normal[0] * penetration_depth;
                other.position[1] += collision_normal[1] * penetration_depth;
                other.position[2] += collision_normal[2] * penetration_depth;
            } else {
                // Both spheres are non-static, adjust both positions
                let correction_factor = penetration_depth / (self.radius + other.radius);
                self.position[0] -= collision_normal[0] * self.radius * correction_factor;
                self.position[1] -= collision_normal[1] * self.radius * correction_factor;
                self.position[2] -= collision_normal[2] * self.radius * correction_factor;
                other.position[0] += collision_normal[0] * other.radius * correction_factor;
                other.position[1] += collision_normal[1] * other.radius * correction_factor;
                other.position[2] += collision_normal[2] * other.radius * correction_factor;
            }
        }
    }
}
...
pub fn update(&mut self, delta_time: f32, spheres: &mut [Sphere]) {
    ...
    fn distance_between_spheres(s1: &Sphere, s2: &Sphere) -> f32 {
